...
//OpenPostgresConnection opens a postgres connection using the SQL ORM library Gorm. Example connString
//format:
...
//QueryAllFromDatabase uses a given DB connection to query all of type desiredStruct from the database.
//Expects a slice.
...
//InsertIntoDatabase uses a given DB connection to insert a pointer into a database. Uses the type of the passed
...
func InsertIntoDatabase(db *gorm.DB, ptrToInsert interface{}) error {
	if nil == ptrToInsert || nil == db {
		return fmt.Errorf("input object(s) nil, %v %v", db, ptrToInsert)
	}
